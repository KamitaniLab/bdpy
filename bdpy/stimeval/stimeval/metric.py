...
metric.py
    ...
    The x and y must have the same shape,
     and the first dimension must be a batch size (sample).
...
def corr(x, y, var='col'):
    """Calculate correaltion between x and y accoring var argument

    Args:
        x (np.array): target (decoded/recon) values [batch_size, *shape]
        y (np.array): soruce (true) values [batch_size, *shape]
        var (str, optional):
         Specifying whther rows or colmuns represent variables.
         Defaults to 'col'.
    ...
    r = np.array([np.corrcoef(x_flat[i].flatten(), y_flat[i].flatten())[0, 1]
                  for i in tqdm(range(nvar))])
...
def pairwise_identification(x, y):
    """Claculate pairwise identification between x and y.
    ...
    for i, label in enumerate(labels):
        ...
        correct_rate.append((pred_num - np.sum(pred_feat > correct_feat))
                            / pred_num)
    return np.array(correct_rate)


def corrmat(x, y, var='row'):
    """Clacualte correlation matrix between x and y
    ...
    cmat = (np.dot(submean(x), submean(y).T)
            / (nobs - 1)) / np.dot(np.matrix(np.std(x, axis=1, ddof=1)).T,
