...
def __plot_violin(
    ...
    '''
    seabornのviolin plotはmean値の描画制御に対応していないため，これだけmatplotlibベースで描画
    '''
    if group is None:
...
def __strict_data(
    ...
    """
    指定されたsp_labelとfig_labelに限定
    &
    指定されたx_listとgroup_listの組み合わせに限定
    """
...
def __weird_form_to_long(df, target_col, identify_cols=[]):
    """
    target_col 内の array を分解する
    target_col で指定された column および，identify_cols で指定された column set による新しい dataframe を作成する
    """
